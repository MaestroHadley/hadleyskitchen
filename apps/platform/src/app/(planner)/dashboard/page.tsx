...
import { ArrowRight, CalendarBlank, CheckCircle, FileText, Notebook, ShoppingCartSimple, Sparkle } from "@phosphor-icons/react/dist/ssr";
...
export default async function DashboardPage() {
  ...
  return <>
    ...
    <section className="dashboard-grid">
      <article className="panel"><div className="section-heading"><div><p className="eyebrow">Recipe library</p><h2>Recently updated</h2></div><Link className="text-link" href="/recipes">View all <ArrowRight /></Link></div>{recipeResult.recipes.map((recipe) => <Link className="dashboard-row" href={`/recipes/${recipe.id}`} key={recipe.id}><span className="row-icon"><FileText weight="duotone" aria-hidden="true" /></span><span><strong>{recipe.name}</strong><small>{recipe.category} · {recipe.yieldPerBatch} {recipe.yieldLabel}</small></span><ArrowRight /></Link>)}</article>
