import { CheckCircle, LockKey, SlidersHorizontal, WarningCircle } from "@phosphor-icons/react/dist/ssr";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ google?: string; googleReason?: string; saved?: string; appearance?: string; appearanceError?: string }> }) {
  ...
  if (!isDemoMode() && supabase && user) {
    ...
    themeId = resolveThemeId(data?.theme_id);
  }
  return <>
    ...
    {params.appearance && !params.appearanceError && <p className="page-success"><CheckCircle weight="fill" />Your workspace appearance was saved.</p>}
    {params.appearanceError && <p className="action-error"><WarningCircle weight="fill" />We couldn’t save that appearance. Your existing theme is still active.</p>}
    <section className="account-grid"><article className="panel account-card"><span className="feature-icon"><LockKey weight="duotone" /></span><div className="account-card-copy"><p className="eyebrow">Signed in</p><h2>Your account</h2><p className="muted">{user?.email ?? "Local design preview"}</p></div><div className="account-card-action"><LogoutButton /></div></article><DriveConnection connectedFromCallback={params.google === "connected"} failedFromCallback={params.google === "failed"} failureReason={params.googleReason} /></section>
    <form action={savePlannerSettings} className="panel settings-form"><div className="section-heading"><div><p className="eyebrow">Planning defaults</p><h2>Bakery settings</h2><p className="muted">These values become the starting point for new events.</p></div><span className="feature-icon"><SlidersHorizontal /></span></div><div className="form-grid three"><label>Bakery name<input name="bakeryName" defaultValue={bakeryName} /></label><label>Starter hydration<div className="number-suffix"><input name="starterHydration" type="number" min="1" max="300" defaultValue={Math.round(settings.starterHydration * 100)} /><span>%</span></div></label><label>Shopping buffer<div className="number-suffix"><input name="shoppingBuffer" type="number" min="0" max="100" defaultValue={Math.round(settings.shoppingBuffer * 100)} /><span>%</span></div></label><label>Starter seed parts<input name="seedParts" type="number" min="0" step="0.1" defaultValue={settings.starterSeedParts ?? 1} /></label><label>Starter flour parts<input name="flourParts" type="number" min="0" step="0.1" defaultValue={settings.starterFlourParts ?? 2} /></label><label>Starter water parts<input name="waterParts" type="number" min="0" step="0.1" defaultValue={settings.starterWaterParts ?? 2} /></label></div><button className="button primary" type="submit">Save defaults</button></form>
    <AppearancePicker savedTheme={themeId} />
