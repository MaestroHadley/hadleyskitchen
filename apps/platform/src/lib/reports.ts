...
import { formatPoundsOunces } from "./weight";
...
export function buildReportSections(event: PlannerEvent, recipes: Recipe[], settings: PlannerSettings): ReportSection[] {
  ...
  return [
    ...
    { title: "Shopping List", rows: [["Ingredient", "Exact grams", "Exact lb + oz", "Buffered grams", "Buffered lb + oz", "Packages to buy"], ...plan.shopping.map((row) => [row.name, Math.round(row.exact), formatPoundsOunces(row.exact), Math.round(row.buffered), formatPoundsOunces(row.buffered), row.packages ?? ""])] },
