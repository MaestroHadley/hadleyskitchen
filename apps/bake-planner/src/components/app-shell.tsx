  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      <nav aria-label="Planner navigation">
        {navigation.slice(0, 3).map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={isActive(href) ? "nav-link active" : "nav-link"}><Icon weight={isActive(href) ? "fill" : "regular"} /><span>{label}</span></Link>)}
      </nav>
