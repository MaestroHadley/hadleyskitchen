...
export async function GET(request: NextRequest) {
  ...
  if (!code || !supabase || !data.user) return NextResponse.redirect(new URL("/?google=failed", url.origin));
  ...
  if (!tokenResponse.ok || !tokens.refresh_token) return NextResponse.redirect(new URL("/?google=failed", url.origin));
